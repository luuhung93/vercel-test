...
import { getAllReviewsPosts } from "../../libs/api/categories"

import { getPost, getRelatedPostsForPostInReviewCat } from "../../libs/api/post"
...
const Single: React.VFC<{ post: PostProps, posts: PostProps[] }> = ({ post, posts }) => {
	...
	let articleSection = post?.tags?.nodes[0] ? [post?.tags?.nodes[0]?.name, "Reviews"] : ["Reviews"]
...
export const getStaticPaths: GetStaticPaths = async () => {
	const posts = await getAllReviewsPosts();
	return {
		paths: posts.map((e: { slug: string }) => '/reviews/' + e.slug) || [],
		fallback: "blocking",
	};
};
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
	...
	const posts: PostProps[] = tagId ? await getRelatedPostsForPostInReviewCat(String(post?.tags?.nodes[0]?.tagId), [String(post?.postId)]) : [];
