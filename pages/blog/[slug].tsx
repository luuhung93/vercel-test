...
import { getAllBlogPosts } from "../../libs/api/categories"

import { getPost, getRelatedPostsForPostInBlogCat } from "../../libs/api/post"
...
const Single: React.VFC<{ post: PostProps, posts: PostProps[] }> = ({ post, posts }) => {
	...
	let articleSection = post?.tags?.nodes[0] ? [post?.tags?.nodes[0]?.name, "How To"] : ["How To"]
...
export const getStaticPaths: GetStaticPaths = async () => {
	...
	return {
		paths: posts.map((e: { slug: string }) => '/blog/' + e.slug + "/") || [],
		fallback: "blocking",
	};
};
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
	...
	const posts: PostProps[] = await getRelatedPostsForPostInBlogCat(tagId ? String(tagId) : undefined, [String(post?.postId)]);
